...
class WorkloadGenerator:
    def __init__(self, args):
        ...
        print(f"disable_random_sample {args.disable_random_sample}")
        ...
        # Replace all strings with repeating "abcd" pattern while maintaining original length
        print(f"len(self.candidate_inputs) {len(self.candidate_inputs)}")
        print(f"len(self.candidate_inputs[0]) {len(self.candidate_inputs[0])}")
        for i in range(1):
            print(
                f"input[{i}] len {len(self.candidate_inputs[i])} {self.candidate_inputs[i][:10]}"
            )
            tokens = self.tokenizer.encode(self.candidate_inputs[i])
            print(f"tokens len {len(tokens)}")
            print(f"tokens {tokens[:10]} ... {tokens[-10:]}")
    ...
    def response_handler(self):
        while True:
            try:
                ...
                print(
                    f"Response received for client {client_id}, text {response.generated_text}"
                )
    ...
    def run(self):
        ...
        performance_data = {
            "summary": {
                ...
                "time_duration": self.finished_time - self.start_time,
            },
        }
        ...
        print(
            f"  Time duration: {performance_data['summary']['time_duration']:.2f} seconds"
        )
