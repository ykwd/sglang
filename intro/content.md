...
数据预取是HiCache的核心优化技术之一，旨在提前将L3存储中的KV缓存加载到L2主机内存中，以减少后续访问时的延迟。预取算法采用多层次的策略和优化机制。
...
**多线程、异步计算**： 数据预取采用了多线程、异步计算的方式提高预取效率。线程`prefetch_thread_func`会不断从`prefetch_queue`中取出可能需要预取操作的Prefill请求，并查询L3得到C2，若C2超过阈值，则会放入等待预取的`prefetch_buffer`。` prefetch_io_aux_func`线程则会不断从`prefetch_buffer`中取出并执行预取请求。对于需要预取的数据，Mooncake会通过传输速度可达每秒数十GB甚至更多的RDMA，并行地从远程存储节点中读取数据。这个设计让数据预取可以高效快速地完成。
...
我们发现，在实际部署中，timeout策略可能相对来说更为实用。一方面，预取延迟不确定；另一方面，预取截止时间受到SLO（服务级别目标）的约束。因此，允许用户灵活配置预取超时参数，并根据每个请求的预取token数量动态确定超时值，具有很高的实用价值。为此，HiCache引入了2个配置参数来支持精细地控制预取超时条件：
...
数据写回机制负责将频繁访问的KV缓存从高速层迁移到低速层进行长期存储，实现跨实例的缓存共享和容量扩展。

**写回策略**：HiCache支持三种写入策略：
- **write_through**：写透模式，每次访问都立即写回到下一层
- **write_through_selective**：选择性写透模式，基于访问频率阈值决定是否写回
- **write_back**：写回模式，在驱逐时才写回到下一层

**智能写回算法**：
1. **访问频率跟踪**：系统为每个节点维护访问计数器，跟踪其被访问的频率
2. **阈值触发**：当节点的访问次数达到写透阈值（默认1-2次）时，触发写回操作
3. **分层写回**：L1中的热点数据写回到L2，L2中的热点数据写回到L3（Mooncake）
4. **异步执行**：写回操作在独立的备份线程中异步执行，避免阻塞推理流程

**写回优化技术**：
- **批量操作**：采用批量写回策略，将多个页面的数据一次性写入存储后端，提高I/O效率
- **零拷贝传输**：对于支持的后端（如Mooncake、3FS），采用零拷贝机制直接传输内存指针，避免数据拷贝开销
- **内存保护**：在写回过程中，通过引用计数机制保护正在写回的数据，防止被意外驱逐
- **错误处理**：支持部分成功和重试机制，确保写回操作的可靠性

**跨实例共享**：通过将KV缓存写回到L3分布式存储，实现了跨SGLang实例的缓存共享。每个缓存的KV都可以被集群中的所有实例访问，在相同的内存预算下显著提升了缓存命中率。
