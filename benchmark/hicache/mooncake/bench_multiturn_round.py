...
class WorkloadGenerator:
    def __init__(self, args):
        ...
        init_requests = [
            (i, gen_payload(self.candidate_inputs[i], args.output_length), 0)
    ...
    async def handle_request(self, item):
        try:
            client_id, payload, current_round = item
            response = await async_request_sglang_generate(payload, self.url, self.pbar)
            if self.pbar.n == self.pbar.total:
                self.finished_time = time.perf_counter()
            self.response_queue.put((client_id, response, current_round))
        except Exception as e:
            print(f"Request failed: {e}")
    ...
    def run(self):
        ...
        # Print round-based TTFT summary
        print("\nRound-based TTFT summary:")
        for round_num in range(args.num_rounds):
            if self.round_ttft[round_num]:
                avg_ttft = sum(self.round_ttft[round_num]) / len(
                    self.round_ttft[round_num]
                )
                print(
                    f"  Round {round_num}: Average TTFT = {avg_ttft:.2f}s ({len(self.round_ttft[round_num])} requests)"
                )
            else:
